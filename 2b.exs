defmodule Day2Puzzle2 do
  ...
  def find_box(box_ids) do
    box_ids
    |> Stream.unfold(fn
      [] -> nil
      [h|t] -> {{h, t}, t}
    end)
    |> Stream.flat_map(fn {h, t} -> check_one_box_id(h, t) end)
    |> Enum.to_list()
  end

  defp check_one_box_id(h, t) do
    t
    |> Task.async_stream(fn x ->
      cond do
        Enum.count(h) == Enum.count(x) -> diff(h, x)
        true -> nil
      end
    end)
    |> Enum.to_list()
  ...
  defp diff(a, b) do
    ...
    cond do
      Enum.count(same) == Enum.count(a) - 1 -> same
      true -> nil
    end
  end
